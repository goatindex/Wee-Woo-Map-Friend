...
export class UtilityManager {
  constructor(
    ...
    this.moduleLogger.info('UtilityManager initialized');
  }
  ...
  async init(dependencies) {
    const timer = this.moduleLogger.time('init-utility-manager');
    
    try {
      this.moduleLogger.info('Initializing UtilityManager...');
      ...
      this.moduleLogger.info('UtilityManager initialization complete');
      ...
      this.moduleLogger.error('Failed to initialize UtilityManager', {
        error: error.message,
        stack: error.stack
      });
  ...
  getResponsiveContext() {
    const timer = this.moduleLogger.time('get-responsive-context');
    ...
    try {
      ...
      this.moduleLogger.error('Failed to get responsive context', {
        error: error.message,
        stack: error.stack
      });
  ...
  showSidebarError(message) {
    const timer = this.moduleLogger.time('show-sidebar-error');
    
    try {
      this.moduleLogger.error('Sidebar Error:', { message });
      ...
      if (sidebar) {
        ...
        this.moduleLogger.info('Sidebar error displayed', { message: message.substring(0, 100) });
      } else {
        timer.end({ reason: 'no-sidebar', success: true });
        this.moduleLogger.warn('Sidebar not found for error display', { message });
      }
      ...
      this.moduleLogger.error('Failed to show sidebar error', {
        error: error.message,
  ...
  convertMGA94ToLatLon(easting, northing) {
    const timer = this.moduleLogger.time('convert-mga94-to-latlon');
    ...
    try {
      ...
      this.moduleLogger.warn('MGA94 coordinate conversion not implemented - using approximate values');
      ...
      this.moduleLogger.error('Failed to convert MGA94 coordinates', {
        error: error.message,
        easting,
        northing
      });
  ...
  createCheckbox(id, label, checked, onChange) {
    const timer = this.moduleLogger.time('create-checkbox');
    ...
    try {
      ...
      this.moduleLogger.debug('Checkbox created', { id, label: label.substring(0, 50) });
      ...
      this.moduleLogger.error('Failed to create checkbox', {
        error: error.message,
        id,
        label
      });
   ...
   */
  destroy() {
    const timer = this.moduleLogger.time('destroy-utility-manager');
    ...
    try {
      ...
      this.moduleLogger.info('UtilityManager destroyed');
      ...
      this.moduleLogger.error('Failed to destroy UtilityManager', {
        error: error.message
      });
    }
  }
