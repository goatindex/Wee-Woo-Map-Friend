...
export class DataValidator {
  constructor(
    ...
    this.moduleLogger.info('DataValidator initialized');
  ...
  async validateGeoJSON(geojson, options = {}) {
    const timer = this.moduleLogger.time('geojson-validation');
    const startTime = performance.now();
    
    try {
      this.moduleLogger.info('Starting GeoJSON validation', { 
        type: geojson?.type,
      ...
      this.moduleLogger.info('GeoJSON validation completed', {
        valid: result.valid,
        featureCount: result.featureCount,
        errorCount: result.errors.length,
        warningCount: result.warnings.length
      ...
      this.moduleLogger.error('GeoJSON validation failed', { error: error.message });
  ...
  async validateSchema(data, schemaName, options = {}) {
    const timer = this.moduleLogger.time('schema-validation');
    const startTime = performance.now();
    
    try {
      this.moduleLogger.debug('Starting schema validation', { schemaName, dataType: typeof data });
      ...
      for (const [ruleName, rule] of schema.rules) {
        try {
          ...
          this.moduleLogger.warn('Schema validation rule failed', { 
            schemaName, 
            ruleName, 
      ...
      timer.end({ error: error.message, success: false });
      this.moduleLogger.error('Schema validation failed', { schemaName, error: error.message });
  ...
  sanitizeData(data, options = {}) {
    const timer = this.moduleLogger.time('data-sanitization');
    
    try {
      this.moduleLogger.debug('Starting data sanitization', { dataType: typeof data });
      ...
      this.moduleLogger.debug('Data sanitization completed');
      return sanitized;
      
    } catch (error) {
      timer.end({ error: error.message, success: false });
      this.moduleLogger.error('Data sanitization failed', { error: error.message });
  ...
  registerRule(name, rule) {
    ...
    this.moduleLogger.debug('Validation rule registered', { name, level: rule.level });
  }
  ...
  registerSchema(name, schema) {
    ...
    this.moduleLogger.debug('Schema registered', { name, ruleCount: schema.rules?.size || 0 });
  }
  ...
  resetStatistics() {
    ...
    this.moduleLogger.info('Validation statistics reset');
  }
  ...
  async handleValidationRequest(payload) {
    try {
      ...
      if (type === 'geojson') {
        ...
        this.moduleLogger.warn('Unknown validation type requested', { type });
        return;
      }
      ...
      this.moduleLogger.error('Validation request handling failed', { 
        error: error.message,
        payload 
      });
    }
