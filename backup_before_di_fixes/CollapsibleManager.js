 ...
 */
@injectable()
export class CollapsibleManager extends BaseService {
  constructor(
    ...
    this.logger.info('Collapsible management system initialized');
  }
  ...
  async init() {
    ...
    try {
      ...
      this.logger.info('Collapsible management system ready');
  ...
  setupEventListeners() {
    ...
    this.eventBus.on('ui:sectionAdded', ({ headerId, listId, expanded }) => {
      this.setupCollapsible(headerId, listId, expanded);
    });
    
    // Listen for section state changes
    this.eventBus.on('ui:sectionStateChange', ({ headerId, expanded }) => {
  ...
  setupCollapsible(headerId, listId, expanded = false) {
    this.logger.debug('setupCollapsible called', { headerId, listId, expanded });
    
    const header = document.getElementById(headerId);
    const list = document.getElementById(listId);
    
    this.logger.debug('setupCollapsible elements found', { 
      header: !!header, 
      list: !!list,
    ...
    if (!header || !list) {
      this.logger.error('Missing elements, returning early');
      return;
    }
    
    this.logger.debug('Setting up collapsible for', { headerId });
    ...
    header.addEventListener('click', () => {
      this.logger.debug('Header clicked', { headerId });
    ...
    this.logger.debug('Event listener attached to', { headerId });
    ...
    this.logger.info('Collapsible setup complete for', { headerId });
  }
  ...
  toggleSection(headerId) {
    ...
    if (!section) {
      this.logger.warn('Section not found', { headerId });
      return;
    }
    ...
    if (headerId === 'activeHeader') {
      ...
      if (hasActiveItems && header.classList.contains('collapsed')) {
        ...
        this.logger.debug('Preventing collapse of activeHeader - has active items');
        header.classList.remove('collapsed');
        list.style.display = '';
    ...
    this.logger.debug('After toggle', {
  ...
  updateStickyClasses() {
    ...
    this.logger.debug('Sticky classes updated', {
...
export const collapsibleManager = () => {
  console.warn('collapsibleManager: Legacy function called. Use DI container to get CollapsibleManager instance.');
  throw new Error('Legacy function not available. Use DI container to get CollapsibleManager instance.');
};
