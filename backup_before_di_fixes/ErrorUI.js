...
export class ErrorUI {
  constructor(
    ...
    this.moduleLogger.info('ErrorUI initialized');
  }
  ...
  showSidebarError(message, options = {}) {
    ...
    if (!sidebar) {
      this.moduleLogger.warn('Sidebar element not found, cannot display error message', {
        message,
        type
      });
      return;
    ...
    if (id && this.errorMessages.has(id)) {
      this.moduleLogger.debug('Duplicate error message prevented', { id, message });
      return;
    }

    const timer = this.moduleLogger.time('error-message-display');
    ...
    try {
      ...
      this.moduleLogger.info('Error message displayed', {
        message,
        type,
        autoHide,
        id
      ...
      this.moduleLogger.error('Failed to display error message', {
        error: error.message,
        message,
        type,
        stack: error.stack
  ...
  removeErrorMessage(errorElement, id = null) {
    try {
      if (errorElement && errorElement.parentNode) {
        ...
        this.moduleLogger.debug('Error message removed', { id });
      }
    } catch (error) {
      this.moduleLogger.error('Failed to remove error message', {
        error: error.message,
        id,
        stack: error.stack
      });
  ...
  clearAllErrors() {
    ...
    this.moduleLogger.info('All error messages cleared', {
  ...
  isOffline() {
    ...
    this.moduleLogger.debug('Offline status checked', {
      isOffline: offline,
  ...
  setupOfflineListener() {
    if (this.isOfflineListenerSetup) {
      this.moduleLogger.debug('Offline listener already setup');
      return;
    }

    try {
      window.addEventListener('offline', () => {
        this.moduleLogger.info('Application went offline');
      ...
      window.addEventListener('online', () => {
        this.moduleLogger.info('Application came back online');
      ...
      this.moduleLogger.info('Offline listener setup completed');
      
    } catch (error) {
      this.moduleLogger.error('Failed to setup offline listener', {
        error: error.message,
        stack: error.stack
      });
    }
