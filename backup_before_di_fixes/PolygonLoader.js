...
export class PolygonLoader {
  constructor(
    ...
    this.moduleLogger.info('Polygon loading system initialized');
  }
  ...
  async init() {
    ...
    try {
      this.moduleLogger.info('Starting polygon loader initialization...');
      ...
      this.moduleLogger.info('Polygon loading system ready');
  ...
  updateState(category, processedData, layers) {
    try {
      ...
      this.moduleLogger.info(`State updated for category ${category}`);
      
    } catch (error) {
      this.moduleLogger.error(`Failed to update state for ${category}`, { 
        error: error.message, 
        stack: error.stack 
      });
      throw error;
  ...
  async handleShowAllToggle(category, checked) {
    try {
      ...
      if (!bulkStarted) {
        this.moduleLogger.warn('Bulk operation already active, proceeding without bulk optimization');
      }
        ...
        for (let i = 0; i < categoryData.names.length; i += batchSize) {
          ...
          batch.forEach(displayName => {
            ...
            if (rowCb) {
              ...
              this.moduleLogger.debug(`Updated checkbox ${category}_${key}: ${checked} (bulk operation)`);
            } else {
              this.moduleLogger.warn(`No checkbox found for ${category}_${key}`);
            }
      ...
      this.moduleLogger.info(`Show All toggle handled for ${category}`, { 
        checked, 
        itemCount: categoryData.names.length 
      });
      
    } catch (error) {
      this.moduleLogger.error(`Failed to handle Show All toggle for ${category}`, { 
        error: error.message, 
        stack: error.stack 
      });
    }
  ...
  handleFeatureToggleDirect(category, key, checked, displayName) {
    try {
      ...
      this.moduleLogger.warn(`Feature toggle direct failed for ${category}/${key}`, { 
        error: error.message 
      });
    }
  }
  ...
  handleSpecialCategoryLogic(category, key, showing) {
    try {
      ...
      this.moduleLogger.warn(`Special category logic failed for ${category}/${key}`, { 
        error: error.message 
      });
    }
  }
  ...
  showSesChevron(key, map = null) {
    try {
      ...
      this.moduleLogger.debug(`SES chevron shown for ${key}`);
      
    } catch (error) {
      this.moduleLogger.warn(`Failed to show SES chevron for ${key}`, { 
        error: error.message 
      });
    }
  }
  ...
  hideSesChevron(key, map = null) {
    try {
      ...
      if (marker) {
        ...
        this.moduleLogger.debug(`SES chevron hidden for ${key}`);
      }
      
    } catch (error) {
      this.moduleLogger.warn(`Failed to hide SES chevron for ${key}`, { 
        error: error.message 
      });
    }
  }
  ...
  addPolygonPlus(map, layer) {
    try {
      if (window.addPolygonPlus) {
        ...
        this.moduleLogger.debug('Polygon plus added');
      }
    } catch (error) {
      this.moduleLogger.warn('Failed to add polygon plus', { 
        error: error.message 
      });
    }
  }
  ...
  removePolygonPlus(layer, map) {
    try {
      if (window.removePolygonPlus) {
        ...
        this.moduleLogger.debug('Polygon plus removed');
      }
    } catch (error) {
      this.moduleLogger.warn('Failed to remove polygon plus', { 
        error: error.message 
      });
    }
  }
