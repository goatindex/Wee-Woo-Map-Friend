...
export class AmbulanceLoader {
  constructor(
    ...
    this.moduleLogger.info('Ambulance loading system initialized');
  }
  ...
  async init() {
    ...
    try {
      this.moduleLogger.info('Starting ambulance loader initialization...');
      ...
      this.moduleLogger.info('Ambulance loading system ready');
  ...
  setupEventListeners() {
    ...
    this.eventBus.on('layer:ready', () => {
      this.moduleLogger.info('Layer manager ready, can load ambulance stations');
    });
    
    // Listen for configuration changes
    this.eventBus.on('config:change', ({ path, value }) => {
      if (path.startsWith('categoryMeta.ambulance')) {
        this.moduleLogger.info('Ambulance metadata updated');
      }
    });
  ...
  async getFeatures() {
    ...
    try {
      ...
      this.moduleLogger.info(`Loaded ${this.ambulanceData.length} ambulance features`);
      return this.ambulanceData;
      
    } catch (error) {
      this.moduleLogger.error('Error loading ambulance features', { 
        error: error.message 
      });
      return [];
    }
  ...
  async load() {
    ...
    try {
      this.moduleLogger.info('Loading ambulance stations...');
      ...
      this.moduleLogger.info('Ambulance stations loaded successfully', {
        featureCount: this.ambulanceData.length
      });
      
    } catch (error) {
      this.moduleLogger.error('Failed to load ambulance stations', { 
        error: error.message, 
        stack: error.stack 
      });
  ...
  processFeatures() {
    ...
    this.moduleLogger.info(`Processed ${this.ambulanceData.length} ambulance features`);
  }
  ...
  populateSidebar() {
    ...
    if (!listEl) {
      this.moduleLogger.error('ambulanceList element not found in DOM');
      return;
    }
    ...
    this.moduleLogger.info(`Sidebar populated with ${namesByCategory[category].length} ambulance stations`);
  }
  ...
  showMarker(key) {
    ...
    this.moduleLogger.debug(`Ambulance marker shown for ${key}`);
  }
  ...
  hideMarker(key) {
    ...
    if (marker) {
      ...
      this.moduleLogger.debug(`Ambulance marker hidden for ${key}`);
    }
